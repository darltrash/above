#pragma language glsl3
    ...
    vec4 effect(vec4 _c, sampler2D tex, vec2 uv, vec2 coord) {
        vec2 rcpFrame = vec2(1.0/love_ScreenSize.x, 1.0/love_ScreenSize.y);
        vec3 color = FxaaPixelShader(posPos, tex, rcpFrame);
        vec3 dithered = color * 0.75 + color * dither17(coord, u_frame_index % 4);
        color = mix(color, dithered, 0.05);
